...
public final class WindowController {
	...
	public WindowController(Controller controller, OgreNumber ogreNumber, char[][] savedEditMap) {
		this.controller = controller;
		this.imageLoader = new ImageLoader();
		this.gameStgWdw = new GameSettingsWindow();
		this.createMapWdw = new CreateMapWindow(imageLoader);
	...
	private void loadValidImages() {
		images.put('I', imageLoader.getDoorClosedImg());
		images.put('S', imageLoader.getDoorOpenImg());
		images.put('X', imageLoader.getWallImg());
		images.put(' ', imageLoader.getBlankSpaceImg());
		images.put('k', imageLoader.getKeyImg());

		images.put('H', imageLoader.getHeroImg());
		images.put('A', imageLoader.getHeroArmedImg());
		images.put('K', imageLoader.getHeroImg());

		images.put('G', imageLoader.getGuardImg());
		images.put('0', imageLoader.getOgreImg());
		images.put('8', imageLoader.getOgreStunned());
		images.put('$', imageLoader.getOgreStunned());
		images.put('*', imageLoader.getClubImg());
	}
	...
	// ---------------------
	void newGame() {
		Personality guardPersonality = gameStgWdw.getGuardPersonality();
	...
	void makeMove(char move) {
		controller.makeMove(controller.getMove(move));
	}

	public FoeInfo getOgreNumber() {
	...
	public char[][] generateMap() {
		...
		for (int i = 0; i < imgMap.length; i++) {
			for (int j = 0; j < imgMap[0].length; j++) {
				if (images.containsValue(imgMap[i][j])) {
					charMap[i][j] = Utilities.getKeyByValue(images, imgMap[i][j]);
					if (charMap[i][j] == ' ') {
						counter++;
					}
				}
	...
	private boolean checkNewGame(Personality guardPersonality, GameAmbient gameAmbient) {

		OgreNumber ogreNumber = gameStgWdw.getOgreNumber();
		...
		if (ogreNumber.getRepresentation() != -1 && mapCreated) {
		...
		if (!mapCreated) {
			...
			if (!regex.checkOgreNumber(Integer.toString(ogreNumber.getRepresentation()))) {
	...
	private boolean checkMap(char[][] charMap) {
		...
		if (!checkCharacter('0', 1, 5, charMap))
			return false;
